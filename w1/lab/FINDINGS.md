...
- **[01:55:30] First Silent Signal:** Thuật toán Z-score bắt đầu phát hiện sự gia tăng bất thường, lệch khỏi đường cơ sở của `memory_usage_bytes` tại `cart-service`.
- **[06:30:19] JVM GC Warning:** Trong Logs xuất hiện cảnh báo `GC overhead limit warning: pause=384ms heap=88%`. Bộ nhớ bắt đầu đầy, Garbage Collector làm việc quá sức.
- **[06:32:33] First Error Root Cause:** Xuất hiện log lỗi đầu tiên `ProductCatalogCache eviction failed: heap pressure too high`. (Đây chính là hạt giống của thảm họa).
- **[19:03:30] Multivariate Anomaly (Isolation Forest):** Thuật toán phát hiện sự bất thường diện rộng khi hệ thống bắt đầu quá tải.
- **[19:59:00] First OOM Danger:** Cảnh báo `OutOfMemoryError imminent: available heap < 5%`.
- **[23:04:00] Alert Triggered:** Hệ thống sập, HTTP 5xx rate tăng vọt. `cart-service` bị OOMKilled và Restart Loop.
...
## Evidence
> (Xin vui lòng xem các file hình ảnh và báo cáo đính kèm được xuất ra trong quá trình chạy script)
- `01-multi_service_timeline.png` (Side-by-side Cascade effect)
- `01-correlation_heatmap.png` (Sự tương quan giữa 5xx và upstream timeout)
- `02-multi_metric_anomaly.png` (Thời điểm Z-Score & IF chộp được dị thường)
- `03-log_metrics_fusion.txt` (Kết quả chạy Drain3 Log Clustering)
